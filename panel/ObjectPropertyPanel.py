        ...
        if "type" in bpy.data.objects[current_object]:
            ...
            if object_type == "model":
                ...
                if len(bpy.context.selected_objects) > 0:
                    ...
                    layout.label(text="Mask Optimization")
                    box = layout.box() 
                    row = box.row()
                    row.prop(bpy.data.objects[obj_name].optimizationparas, "stop_criteria")
                    row = box.row()
                    row.prop(bpy.data.objects[obj_name].optimizationparas, "translation_learning_rate")
                    row = box.row()
                    row.prop(bpy.data.objects[obj_name].optimizationparas, "rotation_learning_rate")
                    row = layout.row()
                    row.operator("object_property.maskoptimation")
